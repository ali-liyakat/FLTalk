...
DATA_PATH = "./data/client2_train.csv"
...
# ---------------------------
# Load Local Dataset (model-aware)
# ---------------------------
if MODEL_NAME == "logistic_regression":
    ...
    X_train, y_train, X_test, y_test = load_cifar10_resnet(
        data_dir="./data",
        resize_224=True,
        limit_train=200,   # fast test (optional)
        limit_test=100     # fast test (optional)
    )
...
# ---------------------------
# Helper: Safe train call (supports both sklearn + torch models)
# ---------------------------
def _train_call():
    """
...
for r in range(1, ROUNDS + 1):
    ...
    # 3) Fetch global weights (blocks until server publishes)
    ...
    try:
        # Some models might not implement update_model; ignore if missing
    ...
    if isinstance(out, (tuple, list)):
        ...
        acc = out  # old models return only accuracy
    ...
    # IMPORTANT: always send metrics (not only in TypeError branch)
    payload = {}
